# Script to compute marginal effect models and logistic regression models (FIGURE 2)
# Using MEG Cluster PSD values
...
df=fread('stats_model_data_adj_R5.csv') # cluster PSD MMSE adjusted values exacted from 

df_converter=df[group=='patient']
df_converter=df[!is.na(Converters)]


df_converter$meg_average_cluster_adj <-rowMeans(df_converter[,c("megf16_post_adjust", "megf18_4_post_adjust", "megf22_6_post_adjust", "megf27_9_post_adjust", "megf34_3_post_adjust", "megf35_5_post_adjust", "megf36_8_post_adjust", "megf38_post_adjust")])
...
# MOD3
mod3 <- glm(Converters ~ scale(age) + scale(Edu_years) + scale(MMSE) + scale(meg_average_cluster_adj), data = merged_df, family = binomial(link="logit"))
summary(mod3)
summary(margins(mod3))
summary_marg_mod3 <-summary(margins(mod3))
write.csv(summary_marg_mod3, "mod3_age_edu_mmse_MEG.csv", row.names = FALSE)

# MOD4
mod4 <- glm(Converters ~ scale(age) + scale(Edu_years) + scale(MMSE) + scale(meg_average_cluster_adj) + ratio_Hippocampus_TotalGrayVol, data = merged_df, family = binomial(link="logit"))
summary(mod4)
summary(margins(mod4))
summary_marg_mod4 <-summary(margins(mod4))
write.csv(summary_marg_mod4, "mod4_age_edu_mmse_MEG_MRI.csv", row.names = FALSE)
...
write.csv(roc_coordinates, "roc_data_mod3_MEG.csv", row.names = FALSE)

roc_data_mod4 <- roc(merged_df$Converters, predict(mod4, type = "response"))
roc_coordinates <- coords(roc_data_mod4, "all")
write.csv(roc_coordinates, "roc_data_mod4_MEG.csv", row.names = FALSE)

# Compute sensitivity, specificity, AUC, Accuracy
predicted_labels <- ifelse(predict(mod1, type = "response") > 0.5, 1, 0)
...
bootstrap_auc <- function(model, data, response_col) {
  ...
  for (i in 1:n_boot) {
    ...
    # NOTE: This assumes you're using a glm model. Adjust accordingly for different types.
...
# Compute proportion of bootstrap samples where Model 4 AUC is greater than Model 3 AUC
prop_diff_greater_than_zero <- mean(diff_aucMod4Mod3 > 0)
print(prop_diff_greater_than_zero)
