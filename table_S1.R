...
sdf1$CerebralWhiteMatterVol <- scale(log(df1[,c('CerebralWhiteMatterVol')]))
...
df=fread('stats_model_data.csv') # file containing cluster MEG PSD data head position aligned and MMSE adjusted
df_converter=df[group=='patient']
df_converter=df[!is.na(Converters)]

df_converter$meg_average_cluster_adj <-rowMeans(df_converter[,c("megf16_post_adjust", "megf18_4_post_adjust", "megf22_6_post_adjust", "megf27_9_post_adjust", "megf34_3_post_adjust", "megf35_post_adjust", "megf36_8_post_adjust", "megf38_post_adjust")])
...
# MOD3
mod3 <- glm(Converters ~ scale(age) + scale(Edu_years) + scale(meg_average_cluster_adj), data = merged_df, family = binomial(link="logit"))
summary(mod3)
...
write.csv(summary_marg_mod4, "mod4_age_edu_mmse_MEG_Hipp_ratio.csv", row.names = FALSE)
...
write.csv(roc_coordinates, "roc_data_mod3.csv", row.names = FALSE)

roc_data_mod4 <- roc(merged_df$Converters, predict(mod4, type = "response"))
roc_coordinates <- coords(roc_data_mod4, "all")
write.csv(roc_coordinates, "roc_data_mod4.csv", row.names = FALSE)
...
sensitivity_mod1 <- true_positives / (true_positives + false_negatives)
specificity_mod1 <- true_negatives / (true_negatives + false_positives)
accuracy_mod1 <- (true_positives + true_negatives) / (true_positives + true_negatives + false_positives + false_negatives)
auc_mod1 <- auc(roc_data_mod1)

print(accuracy_mod1)
print(auc_mod1)
print(sensitivity_mod1)
print(specificity_mod1)
